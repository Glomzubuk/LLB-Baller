...
namespace Baller
{
    class TextureHelper
    {
        public static Texture2D LoadTexture(string folderPath)
        {
            if (Directory.Exists(folderPath))
            {
                if (Directory.GetFiles(folderPath).Length > 0)
                {
                    ...
                    {
                        if (filePath.ToLower().EndsWith(".png")) return LoadPNG(filePath);
                        if (filePath.ToLower().EndsWith(".dds")) return LoadDDS(filePath);
                    };
                }
            }
        ...
        {
            DDSImage dds = new DDSImage(File.ReadAllBytes(filePath));


            Texture2D tex = new Texture2D(dds.Width, dds.Height, dds.GetTextureFormat(), true, false);
